...
def idworkbook():
    ...
    for i in ms:
        ...
        nm = ws["A4"].value
        booky.close()
        os.rename(i, f"memstatus/E{nm[-1]}.xlsx")
...
def processing():
    ...
    for i in ms:
        ...
        for j in list(range(1, 10)):
            ...
            MD[i[10:12]]["Lami2"]["Membrane"] = [ws["E13"].value, ws["E14"].value]
            MD[i[10:12]]["Lami3"]["Membrane"] = [ws["E17"].value, ws["E18"].value]
            MD[i[10:12]]["Lami4"]["Membrane"] = [ws["E21"].value, ws["E22"].value]
            # Teflons
            MD[i[10:12]]["Lami1"]["Teflon"][0].append(ws[f"{chr(j + 68)}11"].value)
            MD[i[10:12]]["Lami1"]["Teflon"][1].append(ws[f"{chr(j + 68)}12"].value)

            MD[i[10:12]]["Lami2"]["Teflon"][0].append(ws[f"{chr(j + 68)}15"].value)
            MD[i[10:12]]["Lami2"]["Teflon"][1].append(ws[f"{chr(j + 68)}16"].value)

            MD[i[10:12]]["Lami3"]["Teflon"][0].append(ws[f"{chr(j + 68)}19"].value)
            MD[i[10:12]]["Lami3"]["Teflon"][1].append(ws[f"{chr(j + 68)}20"].value)
